...
void detection_run(const std::string &onnx_file) {
    ...
    mindvision::VideoCapture* mv_capture_ = new mindvision::VideoCapture(
        mindvision::CameraParam(1, mindvision::RESOLUTION_1280_X_800, mindvision::EXPOSURE_2500));

    cv::VideoCapture cap_ = cv::VideoCapture("/home/xx/downloads/mmexport1641612044442.mp4");
    ...
    cv::dnn::Net net;
    net = cv::dnn::readNetFromCaffe("/home/xx/code/KCf/trainData/deploy.prototxt", "/home/xx/code/KCf/trainData/_iter_2000.caffemodel");
    while (true) {
        ...
        if (!img.empty()) {
            ...
            if(!detections.empty()) {
                ...
                for (int i = 0; i < detections.size(); i++) {
                    ...
                    contour.push_back(armor.pts[0]);
                    contour.push_back(armor.pts[1]);
                    contour.push_back(armor.pts[2]);
                    contour.push_back(armor.pts[3]);
